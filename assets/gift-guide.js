...
function renderGiftGuidePopup(product) {
  ...
  // Build color buttons with label
  var colorsHtml = '';
  if (colors.length > 0) {
    colorsHtml =
      '<label class="gift-guide-popup__variant-label">Color</label>' +
      '<div class="gift-guide-popup__colors">' +
      colors
        .map(function (color) {
          return (
            '<button type="button" class="gift-guide-popup__color-btn' +
            (color === selectedColor ? ' selected' : '') +
            '" data-color="' +
            color +
            '">' +
  ...
  // Build size dropdown with label
  var sizesHtml = '';
  if (sizes.length > 0) {
    sizesHtml =
      '<label class="gift-guide-popup__variant-label">Size</label>' +
      '<select class="gift-guide-popup__size-select">' +
  ...
  document.getElementById('gift-guide-popup-dynamic').innerHTML =
    '<div class="gift-guide-popup__top">' +
    '<div class="gift-guide-popup__left">' +
    '<img class="gift-guide-popup__img" src="' +
    ...
    '</div>' +
    '<div class="gift-guide-popup__right">' +
    '<div class="gift-guide-popup__product-title">' +
    product.title +
    '</div>' +
    '<div class="gift-guide-popup__product-price">' +
    ...
    '</div>' +
    '<div class="gift-guide-popup__product-desc">' +
    product.description +
    '</div>' +
    '</div>' +
    '</div>' +
    ...
    '<div class="gift-guide-popup__bottom">' +
    colorsHtml +  // Color first
    sizesHtml +   // Size second
    '<button class="gift-guide-popup__atc-btn" type="button">' +
    'ADD TO CART <span class="gift-guide-popup__atc-arrow">→</span>' +
    '</button>' +
    '</div>';
  ...
  // Add to Cart event
  document
    .querySelector('.gift-guide-popup__atc-btn')
  ...
  function updateSelectedVariant() {
    ...
    if (variant) {
      var priceElem = document.querySelector(
        '.gift-guide-popup__product-price'
      );
      if (window.Shopify && Shopify.formatMoney) {
        priceElem.textContent = Shopify.formatMoney(variant.price);
      } else {
      ...
      if (variant.featured_image && variant.featured_image.src) {
        document.querySelector('.gift-guide-popup__img').src =
